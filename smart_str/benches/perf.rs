...
fn smart_str_cloning(c: &mut Criterion) {
    ...
    c.bench_function("SmartStr cloning", |b| b.iter(|| {
        let clone = og.clone();
...
fn smol_str_cloning(c: &mut Criterion) {
    ...
    c.bench_function("SmolStr cloning", |b| b.iter(|| {
        let clone = og.clone();
        assert_eq!(og.as_str(), clone.as_str());
    }));
}
...
fn smartstring_cloning(c: &mut Criterion) {
    ...
    c.bench_function("SmartString cloning", |b| b.iter(|| {
        let clone = og.clone();
        assert_eq!(og.as_str(), clone.as_str());
    }));
}

criterion_group!(inline, smart_str_inline, smol_str_inline, smartstring_inline);
criterion_group!(heap, smart_str_heap, smol_str_heap, smartstring_heap);
criterion_group!(cloning, smart_str_cloning, smol_str_cloning, smartstring_cloning);
